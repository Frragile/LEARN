...
Component({
  ...
  methods: {
    ...
    display(year, month, date) {
      this.setData({
        year, //year:year, 同名可省略 
        month,
        date,
        title: year + '年' + this.zero(month) + '月' //zero 给个位数月份前面补充 0 
      })
      //
    ...
    // 默认选中当天并且初始化
    today() {
      // defaultValue 有就用 defaultValue 没有就newDate一个
    ...
    select(e) {
      ...
      this.triggerEvent('event', select)
    },
    //上一个月
    lastMonth() {
